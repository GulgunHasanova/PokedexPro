...
class _CompareScreenState extends State<CompareScreen> {
  ...
  Widget build(BuildContext context) {
    if (widget.availablePokemon.isEmpty) {
      return const Center(
        child: Text('Müqayisə üçün pokemon yoxdur.', style: TextStyle(color: Colors.white)),
      );
    }
      ...
      child: Column(
        children: [
          Container(
            ...
            child: Column(
              children: [
                ...
                Text(
                  'İki pokemon seçin və onların güclərini müqayisə edin.',
          ...
          Row(
            children: [
              Expanded(
                child: _buildPokemonSelector(pokemon1, (p) => setState(() => pokemon1 = p), 'Pokemon 1'),
              ),
              const Padding(
                padding: EdgeInsets.symmetric(horizontal: 16),
                child: Text('VS', style: TextStyle(color: Colors.redAccent, fontWeight: FontWeight.bold, fontSize: 18)),
              ),
              Expanded(
                child: _buildPokemonSelector(pokemon2, (p) => setState(() => pokemon2 = p), 'Pokemon 2'),
              ),
            ],
          ),
          const SizedBox(height: 24),
